...
def INT_FLOAT(file, item_list, i):
    ...
    e0 = int(array[0])
    r0 = array[1]
    item_list[i].setValues(item_list[i], classes.indicators["NOTHING"], classes.indicators["NOTHING"], classes.indicators["NOTHING"], classes.indicators["NOTHING"], e0, classes.indicators["NOTHING"], classes.indicators["NOTHING"], classes.indicators["NOTHING"], r0)
...
def INT_FLOAT_FLOAT_FLOAT(file, item_list: classes.item, i):
    ...
    rr = array[2]
    rrr = array[3]
    item_list[i].setValues(item_list[i], e, r, rr, rrr, classes.indicators["NOTHING"], classes.indicators["NOTHING"], classes.indicators["NOTHING"], classes.indicators["NOTHING"], classes.indicators["NOTHING"])
...
def INT_INT_INT_INT_INT(file, item_list, i):
    ...
    e4 = array[3]
    e5 = array[4]
    item_list[i].setValues(item_list[i], e1, classes.indicators["NOTHING"], classes.indicators["NOTHING"], classes.indicators["NOTHING"], e2, e3, e4, e5, classes.indicators["NOTHING"])
...
#    file = open("3dtest.dat", "r")
    ...
    for i in range(n):
        ...
        print("Printing itemList from obtenerDatos "+ str(item_list[i].getX(item_list[i])) + " " + str(item_list[i].getY(item_list[i])) + " " + str(item_list[i].getZ(item_list[i])))

def correctConditions(n, list, indices):
    for i in range(n):
        indices[i] = list[i].getNode1(list[i])
    
    for i in range(n-1):
        pivot = list[i].getNode1(list[i])
        for j in range(i, n):
            if(list[j].getNode1(list[i]) > pivot):
                list[j].setNode1(list[j].getNode1(list[i])-1)
...
#    for i in range(10):
    print("Printing itemList from leerMalla "+ str(item_list[0].getX(item_list[0])) + " " + str(item_list[0].getY(item_list[0])) + " " + str(item_list[0].getZ(item_list[0])))
...
m.node_list[0].setX(m.node_list[0], 9)
print(m.node_list[0].getX(m.node_list[0]))
print(m.getNode(0).getX(m.getNode(0)))
for i in range(m.getSize(classes.sizes["NODES"])):
    print(str(m.getNodes()[i].getX(m.getNodes()[i]))+" "+str(m.getNodes()[i].getY(m.getNodes()[i]))+" "+str(m.getNodes()[i].getZ(m.getNodes()[i])))
